...
/**
 * @see net.minecraft.client.network.ClientPlayerInteractionManager
 */
public class GameInteractionManager {
    ...
    private final PlayNetworkHandler networkHandler;
    ...
    public GameInteractionManager(GameClient client, PlayNetworkHandler networkHandler) {
    ...
    public ClientPlayerEntity createPlayer(ClientWorld world, StatHandler statHandler, ClientRecipeBook recipeBook, boolean lastSneaking, boolean lastSprinting) {
        return new ClientPlayerEntity(
                ...
                world,
                ReflectionUtil.createProxy(ClientPlayNetworkHandler.class, this.networkHandler),
                statHandler,
                recipeBook,
                lastSneaking,
                lastSprinting);
    ...
    public void clickSlot(int syncId, int slotId, int button, SlotActionType actionType, PlayerEntity player) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            ItemStack itemStack2 = defaultedList.get(i).getStack();
            if (!ItemStack.areEqual(itemStack, itemStack2)) {
                int2ObjectMap.put(i, itemStack2.copy());
            }
        }
    ...
    public void processPlayerActionResponse(ClientWorld world, BlockPos pos, BlockState state, PlayerActionC2SPacket.Action action, boolean approved) {
        ...
        while (this.unacknowledgedPlayerActions.size() >= MAX_UNACKNOWLEDGED_PLAYER_ACTIONS) {
